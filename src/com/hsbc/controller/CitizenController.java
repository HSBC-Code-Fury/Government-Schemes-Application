...
public class CitizenController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (header[length].equals("CitizenLogin.html")) {
			RequestDispatcher rd = req.getRequestDispatcher("validatecitizen"); // Redirect to page after login
			rd.forward(req, resp);
		} else if (header[length].equals("CitizenRegistration.html")) {
			RequestDispatcher rd = req.getRequestDispatcher("addcitizen");// Redirect to page after citizen registration
			rd.forward(req, resp);
		} else if (header[length].equals("ApplyScheme.html")) {
			RequestDispatcher rd = req.getRequestDispatcher("applyscheme");// Redirect to page after apply scheme
			rd.forward(req, resp);
		}
		/*
